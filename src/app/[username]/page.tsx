      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="w-full lg:w-2/5 lg:fixed lg:h-screen overflow-y-auto">
          <div className='bg-slate-20 flex flex-col items-center p-2'>
            ...
            <p className='px-5 text-base md:text-lg font-semibold font-sans text-wrap text-center'>{user.bio}</p>
          </div>
          <div className='flex flex-col items-center justify-center'>
            <div className='flex flex-wrap gap-2 p-4 m-2 border border-zinc-400 shadow-orange-30 rounded-3xl w-fit'>
              {user.skills.map((skill: string, index: number) => (
                <span key={index} className='bg-amber-100 shadow-lg shadow-orange-200 px-2 py-1 rounded-lg text-sm md:text-lg font-sans'>{skill}</span>
              ))}
            </div>
            <h1 className='text-lg md:text-xl font-bold text-center shadow-sm rounded-2xl p-2 font-mono'>Skills</h1>
          </div>
        </div>
